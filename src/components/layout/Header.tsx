import { chevronDown, menu, menuBlack } from "@/assets";
...
export function Header({ dark }: { dark?: boolean }) {
  ...
  return (
    <header
      className={`w-full flex items-center px-24 2xl:px-40 py-8 xs:justify-center lg:justify-between relative z-20 border-b ${
    ...
    >
      <div className="relative xs:w-full lg:w-auto justify-center items-center xs:flex lg:block">
        <button onClick={() => setOpenMenu(!openMenu)} className="xs:block absolute -left-14 lg:hidden">
      ...
      onClick={() => setOpenMenu(false)}
        className={`lg:block xs:fixed lg:relative xs:h-screen
          ...
          xs:bg-[#00000099] xs:z-50 lg:z-auto lg:backdrop-blur-0 
          lg:bg-transparent xs:backdrop-blur-sm transition duration-150
          ${openMenu ? "-translate-x-0 delay-100" : "xs:-translate-x-full lg:-translate-x-0"}
          `}
      >
        <ul
          className={`xs:flex-col flex lg:flex-row lg:items-center xs:items-start
        ...
        >
          ...
          <li>
            ...
            >
              Documentos{" "}
              <img src={chevronDown} className="w-3" alt="dropdown" />
            </a>
          </li>
          <li>
            <a
